...
void testJumble() {
	...
	//#####################16EFLTEST UNIT##############################
	//################TEST 1#######################333
	cout << "Test whether capital letters are converted to lowercase:" << endl;
	// While debugging, you can use capital letters to make the hidden string easier to locate
	JumblePuzzle jp5("PUMPKIN", "easy");
	showJumble(jp5.getJumble(), jp5.getSize());

//#######################TEST 2############################
//REMOVE // FROM /* AND */ BELOW TO TEST FOUL LANGUAGE CATHCHER

///* 
	cout << "Testing that code does not except non alphabetic letter:" << endl;
	cout << "Inputting !234@" << endl;
	// While debugging, you can use capital letters to make the hidden string easier to locate
	
	try {
		 JumblePuzzle jp6("!234@", "easy");
	}
	catch (BadJumbleException& e) {
		cerr << e.what();
		return;
	...
	//#######################TEST 3############################
	///*
	cout << "Testing whether any of listed swear words are blocked:" << endl;
	cout << "Input bad word poop:" << endl;
	// While debugging, you can use capital letters to make the hidden string easier to locate
	try {
		JumblePuzzle jp7("poop", "easy");	
	}
	catch (BadJumbleException& e) {
		cerr << e.what();
		return;
